...
public class Camera2BasicFragment1 extends Fragment
    ...
    private static boolean currentlyProcessingPicture = false;

    //private static int REQUEST_PERMISSION_CAMERA_STORAGE = 0;
    private static int REQUEST_PERMISSION_CAMERA = 0;
    private static int REQUEST_PERMISSION_STORAGE = 1;
    public static final int REQUEST_PERMISSION_DEVICEID = 2;
    ...
    private ToggleButton toggleCrabType;
...
//    }
    ...
    public static Camera2BasicFragment1 newInstance() {
        return new Camera2BasicFragment1();
    }
...
//        view.findViewById(R.id.picture).setOnClickListener(this);
        ...
        toggleCrabType = (ToggleButton) view.findViewById(R.id.toggle_type);
        buttonGallery = (ImageView) view.findViewById(R.id.button_gallery);
        buttonTakePicture = (ImageView) view.findViewById(R.id.button_picture);
        ...
        buttonGallery.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // tell listener to calling activity that intent must be sent
                camera2basicfragmentlistener.onButtonGalleryClick();
            }
        });
...
//                buttonTakePicture.setEnabled(false);
                if(!currentlyProcessingPicture){
                    currentlyProcessingPicture = true;
                    takePicture();
                }else{
                    Snackbar.make(buttonTakePicture, "Please wait for the current photo to finish processing.", Snackbar.LENGTH_SHORT).show();
                }
            }
        });
    }
...
//                    Environment.DIRECTORY_PICTURES);
            ...
            Log.i("Camera2BasicFragment", "FILE IS " + image.getAbsolutePath());
            return image;
        }
...
//                Snackbar.make(buttonTakePicture, "We need to use the camera and the file storage for the app to work.", Snackbar.LENGTH_SHORT).show();
//            }
            if (mTextureView.isAvailable()) {
                openCamera(mTextureView.getWidth(), mTextureView.getHeight());
            } else {
            ...
            super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        } else if(requestCode == REQUEST_PERMISSION_DEVICEID && grantResults.length > 0  && grantResults[0] == PackageManager.PERMISSION_GRANTED){
            saveDeviceIdToSharedPreferences();
            super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        } else if(requestCode == REQUEST_PERMISSION_STORAGE && grantResults.length > 0  && grantResults[0] == PackageManager.PERMISSION_GRANTED){
            super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        } else{
            Snackbar.make(buttonTakePicture, "We need your permission to this service.", Snackbar.LENGTH_LONG).show();
        }
    }
...
//            }
            case R.id.info: {
                ...
                if (null != activity) {
                    new AlertDialog.Builder(activity)
                            .setMessage("Welcome to crab monitoring.")
    ...
    public static class ConfirmationDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            return new AlertDialog.Builder(getActivity())
                    .setMessage("We need to use the camera and file storage for the app to work.")
                    .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        @Override
    ...
    public void onAttach(Context context) {
        ...
        camera2basicfragmentlistener = (Camera2BasicFragmentListener) context;
    }

    @Override
    public void onDetach() {
        super.onDetach();
        camera2basicfragmentlistener = null;
    }

    Camera2BasicFragmentListener camera2basicfragmentlistener;
